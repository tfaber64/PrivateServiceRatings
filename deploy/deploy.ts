...
async function pickLatestFile(files: string[]): Promise<string | null> {
  ...
  for (let i = 1; i < files.length; i++) {
    ...
    if (m > bestMtime) { bestMtime = m; best = files[i]; }
  }
  return best;
}
...
async function resolveTarget(hre: HardhatRuntimeEnvironment): Promise<{ contractName: string; fqn: string; }>{
  ...
  if (envName) {
    ...
    if (!matchFqn) throw new Error(`CONTRACT_NAME='${envName}' not found in artifacts. Candidates: \n` + contractsFqns.join("\n"));
...
const func: DeployFunction = async (hre: HardhatRuntimeEnvironment) => {
  ...
  if (process.env.CONSTRUCTOR_ARGS) {
    try { args = JSON.parse(process.env.CONSTRUCTOR_ARGS); }
    catch (e) { throw new Error(`Failed to parse CONSTRUCTOR_ARGS JSON: ${e}`); }
  }
